  ...
  >
    {#if currentUser}
      ...
      <div class="flex items-center gap-2.5">
        <div class="avatar placeholder">
          <div
            class="bg-primary/10 text-primary border-primary/20 size-7 overflow-hidden rounded-full border text-xs font-bold"
          >
        ...
        <div class="flex flex-col items-start text-left leading-tight">
          <div class="flex items-center gap-1">
            <span class="text-base-content max-w-28 truncate text-xs font-bold"
              >{currentUser.username}</span
            >
          </div>
          <div class="flex items-center gap-1">
            <span
              class={`badge badge-xs ${getProviderBadgeClass(currentUser.provider)} px-1 py-0 font-mono text-[9px]`}
            >
              {currentUser.provider}
            </span>
            {#if isSandbox}
              <span class="badge badge-warning badge-xs px-1 py-0 font-mono text-[8px]">
                Sandbox
              </span>
            {/if}
          </div>
    ...
    {:else}
      ...
      <div class="text-base-content/70 flex items-center gap-2">
        ...
        <span class="text-base-content text-xs font-semibold">Select User</span>
        <ChevronDown class="text-base-content/50 size-3.5" />
      </div>
    {/if}
  </summary>
  ...
  >
          ...
          <div class="flex items-center gap-3">
            ...
            <div class="flex flex-col">
              ...
              <div class="mt-0.5 flex items-center gap-1">
                <span
                  class={`badge ${getProviderBadgeClass(currentUser.provider)} badge-xs font-semibold`}
                >
                  {currentUser.provider}
                </span>
                {#if isSandbox}
                  <span class="badge badge-warning badge-xs font-semibold"> Sandbox Mode </span>
                {/if}
              </div>
            </div>
          </div>

          <span class="badge badge-success badge-xs gap-1 px-2 py-1 text-[10px] font-medium">
            Active
          </span>
        </div>

        <div
          class="text-base-content/60 border-base-300/40 flex items-center justify-between border-t pt-1 text-[11px]"
        >
          <span>Added {formatDate(currentUser.created_at)}</span>
          {#if currentUser.access_token}
            <span class="text-primary flex items-center gap-1 text-[10px]">
    ...
    {:else}
      ...
      >
        ...
        <div class="text-base-content text-xs font-semibold">No Active Profile</div>
    ...
    <div class="space-y-1.5">
      <div
        class="text-base-content/60 flex items-center justify-between px-1 text-[11px] font-semibold tracking-wider uppercase"
      >
      ...
      {#if users.length === 0}
        <div class="text-base-content/50 px-1 py-1 text-xs italic">No user profiles added yet</div>
      {:else}
          ...
          {#each users as user (user.id)}
            ...
            >
              <div class="flex min-w-0 items-center gap-2.5">
                <div class="avatar placeholder">
                  <div
                    class="bg-base-300 text-base-content size-7 overflow-hidden rounded-full text-xs font-bold"
                  >
                    {#if user.avatar_url}
                      <img
                        src={user.avatar_url}
                ...
                <div class="flex min-w-0 flex-col">
                  <span class="truncate text-xs leading-tight font-semibold">{user.username}</span>
                  <div class="flex items-center gap-1">
                    <span class="text-base-content/60 font-mono text-[10px]">{user.provider}</span>
                    {#if isSandbox}
                      <span class="text-warning text-[9px] font-semibold">(Sandbox)</span>
                    {/if}
                  </div>
                </div>
              </div>
    ...
    {#if errorMsg}
      <div class="alert alert-error alert-soft text-error-content rounded-lg p-2 text-xs">
        <span>{errorMsg}</span>
      </div>
    {/if}
    ...
    <button
      ...
      class="btn btn-outline btn-primary btn-sm w-full gap-1.5 rounded-xl text-xs font-medium"
    >
